...
New techniques added Feb 3, 2023 - data augmentation methods on Azerbaijani dataset

Keras implementation of data augmentation methods on the UTI-to-STFT model of
...
for speaker in speakers:
    ...
    for train_valid in ['train', 'valid']:
        ...
        for basefile in ult_files[train_valid]:
            try: 
                with open(basefile + '.augult', 'rb') as f:
                    data = pickle.load(f)
                ult_data = data
                # if not os.path.isfile(basefile + '.augult'):
    ...
    print('ultrasound data size:', ult['train'].shape)
    ...
    def cMaskingU(file): #consecutive time masking for ult data
    ...
    def iMaskingU(file): #intermittent time masking for ult data
            b = read_ult(file, n_lines,n_pixels)
            data = b.copy()
            for i in range(data.shape[0]):
                array = data[i]
                for j in range(array.shape[0]):
                    ...
                    s = len(line)//6
                    e = len(line)//3
                    intervals = [[s, e], [e+s, 2*e], [2*e+s, 3*e]]
                    for start, end in intervals:
                        line[start:end] = 0
                    # random_indices = np.random.choice(np.arange(len(line)), size=n_mask, replace=False)
                    # line[random_indices] = -1
            return data
    ...
    def saveDA():
        for basefile in ult_files_all:
            # data = cMaskingU(basefile + '.ult')
            # data = iMaskingU(basefile + '.ult')
            data = sNoiseI_U(basefile + '.ult')
            # data = rScalingU(basefile + '.ult')
            # data = aMaskingU(basefile + '.ult')
            with open(basefile + '.augult', 'wb') as f:
                pickle.dump(data, f)    
        return 0    



    # delete_files('/data/data_SSI2022/aze_spkr01/')
    # saveDA()
    # saveNoise()
    ...
    history = model.fit(ult['train'], melspec['train'],
                            epochs = 15, batch_size = 128, shuffle = True, verbose = 1,
